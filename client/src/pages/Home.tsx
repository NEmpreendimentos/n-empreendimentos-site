...
const projects = [
  { type: "Landing page", title: "Vértice Studio", meta: "Posicionamento + captação", image: "/manus-storage/n-portfolio-landing_71aaa46a.png", accent: "#c8ff4a" },
  { type: "Loja virtual", title: "Lume Objects", meta: "Catálogo + conversão", image: "/manus-storage/n-portfolio-commerce_871578aa.png", accent: "#8f7cff" },
  { type: "Institucional", title: "Aresta Arquitetura", meta: "Autoridade + presença", image: "/manus-storage/n-portfolio-institutional_d310d31f.png", accent: "#62d8ff" },
];
  ...
  return (
    <div className="site-shell">
      ...
      <main>
        ...
        <section id="beneficios" className="section-pad benefits-section">
          <div className="container"><div className="section-intro split-intro"><div><span className="section-index">N/ 01 — por que comigo</span><h2>Design bom chama atenção.<br /><span>Design estratégico</span> move.</h2></div><p>Do primeiro wireframe ao último detalhe no celular, cada escolha tem um motivo: tornar seu negócio mais claro, desejável e fácil de escolher.</p></div><div className="benefit-grid">{benefits.map(({ icon: Icon, index, title, text }) => <article className="benefit-card" key={title}><div className="card-topline"><span>{index}</span><Icon size={20} /></div><h3>{title}</h3><p>{text}</p><ArrowUpRight className="card-arrow" size={19} /></article>)}</div></div>
        </section>

        <section id="portfolio" className="section-pad portfolio-section">
          <div className="container"><div className="section-intro"><span className="section-index">N/ 02 — seleção de projetos</span><h2>Projetos que sabem<br /><span>onde querem chegar.</span></h2><p>Alguns universos fictícios para mostrar como uma boa interface muda a percepção de valor — e abre espaço para a próxima conversa.</p></div><div className="project-list">{projects.map((project, idx) => <article className={`project-card project-${idx + 1}`} key={project.title}><div className="project-info"><span className="project-type" style={{ color: project.accent }}>{project.type}</span><h3>{project.title}</h3><p>{project.meta}</p><a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="project-link">Quero algo assim <ArrowUpRight size={16} /></a></div><div className="project-image-wrap"><img src={project.image} alt={`Mockup de ${project.title}`} /><div className="image-shade" /></div></article>)}</div></div>
        </section>

        <section id="processo" className="section-pad process-section"><div className="container process-layout"><div className="process-sticky"><span className="section-index">N/ 03 — o caminho</span><h2>Do “preciso de um site” ao <span>“agora faz sentido.”</span></h2><p>Um processo enxuto, transparente e sem palavras difíceis para você acompanhar o projeto com segurança.</p><WhatsappButton secondary>Falar sobre meu projeto</WhatsappButton></div><div className="steps-list">{steps.map((step, idx) => <div className="step" key={step.number}><div className="step-marker"><span>{step.number}</span>{idx < steps.length - 1 && <i />}</div><div><h3>{step.title}</h3><p>{step.text}</p></div></div>)}</div></div></section>

        <section className="section-pad proof-section"><div className="container proof-layout"><div><span className="section-index">N/ 04 — prova social</span><h2>Projetos reais.<br /><span>Resultados que ficam.</span></h2></div><div className="proof-note"><ShieldCheck size={20} /><p>Alguns trabalhos desenvolvidos pela N. Empreendimentos, com textos publicados nas próprias páginas dos projetos.</p></div></div><div className="container testimonial-placeholders"><article className="testimonial-placeholder case-card"><div className="case-card-top"><span className="case-brand">M</span><span className="case-label">MetaBoost · case real</span></div><blockquote>“Depois de anos lutando contra o efeito sanfona e tentando de tudo, finalmente encontrei algo que tratou a raiz do problema.”</blockquote><div className="case-result"><strong>−18 kg</strong><span>em 4 meses</span></div></article><article className="testimonial-placeholder active-placeholder case-card"><div className="case-card-top"><span className="case-brand empreste-brand">E+</span><span className="case-label">EmpresteMais+ · case real</span></div><blockquote>“Liberdade financeira sem burocracia.”</blockquote><div className="case-result"><strong>+50 mil</strong><span>aprovações exibidas no projeto</span></div></article><article className="testimonial-placeholder case-card"><div className="case-card-top"><span className="case-brand">N</span><span className="case-label">Seu negócio · próximo case</span></div><blockquote>Seu resultado pode ser a próxima história contada aqui.</blockquote><a className="project-link" href={WHATSAPP_URL} target="_blank" rel="noreferrer">Conversar sobre meu projeto <ArrowUpRight size={16} /></a></article></div></section>

        <section id="faq" className="section-pad faq-section"><div className="container faq-layout"><div className="faq-heading"><span className="section-index">N/ 05 — perguntas frequentes</span><h2>Antes de começar,<br /><span>vamos deixar claro.</span></h2><p>Se a sua dúvida não apareceu aqui, me chama no WhatsApp. Eu respondo sem rodeios.</p><WhatsappButton secondary>Tirar uma dúvida</WhatsappButton></div><div className="faq-list">{faqs.map((faq, index) => <div className={`faq-item ${openFaq === index ? "is-open" : ""}`} key={faq.question}><button onClick={() => setOpenFaq(openFaq === index ? null : index)} aria-expanded={openFaq === index}><span>{faq.question}</span><ChevronDown size={19} /></button><div className="faq-answer"><p>{faq.answer}</p></div></div>)}</div></div></section>
